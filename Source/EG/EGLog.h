...
#define LOG_JM(LogCategory, Verbosity, Format, ...) UE_LOG(LogCategory, Verbosity, TEXT("%s : %s"), FUNCTION_TCHAR, *FString::Printf(Format, ##__VA_ARGS__))
#define LOG_JM_NET(LogCategory, Verbosity, Format, ...) UE_LOG(LogCategory, Verbosity, TEXT("[%s] %s %s"), NETMODE_TCHAR, FUNCTION_TCHAR, *FString::Printf(Format, ##__VA_ARGS__))

/* DedicatedX 로그 예시(참고용) */
/*
